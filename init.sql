...
-- Testing view to see all accounts and their metadata
-- This view is not used in service directly!
...
COALESCE (last_index + 1, 0) AS next_transfer_index
-- Get all account ids
FROM (SELECT id FROM account) a
-- Join with sum of all incoming transfers
LEFT JOIN (
SELECT destination AS id, SUM (amount) AS incoming_sum
FROM transfer GROUP BY destination
) i ON i.id = a.id
-- Join with sum of all outgoing transfers and last transfer index
LEFT JOIN (
SELECT source AS id, SUM (amount) AS outgoing_sum, MAX (index) AS last_index
FROM transfer
GROUP BY source) o ON o.id = a.id ;
