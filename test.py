...
def test_should_make_transfer_properly():
    with context_client() as client:
        ...
        _, res = make_transfer(
            client=client, source=first_id, destination=second_id, amount=1
        )
        assert res.status_code == 204
...
def test_should_not_make_transfer_if_source_account_is_service_one():
    with context_client() as client:
        ...
        assert res.status_code == 400
        assert res.json() == error_response(
            "the service account cannot be use as a source account"
        )
...
def test_should_not_make_transfer_if_source_and_destination_are_the_same():
    with context_client() as client:
        ...
        _, res = make_transfer(
            client=client, source=first_id, destination=first_id, amount=1
        )
        assert res.status_code == 400
        assert res.json() == error_response(
            "source account must not be equal to the destination account"
        )
...
def test_should_not_make_transfer_if_destination_account_does_not_exist():
    with context_client() as client:
        ...
        _, res = make_transfer(
            client=client, source=first_id, destination=random_id, amount=1
        )
        assert res.status_code == 400
...
def test_should_not_make_transfer_if_source_account_does_not_exist():
    with context_client() as client:
        ...
        _, res = make_transfer(
            client=client, source=random_id, destination=first_id, amount=1
        )
        assert res.status_code == 400
...
def test_should_not_make_transfer_if_amount_is_more_than_source_balance():
    with context_client() as client:
        ...
        amount = balance + 1
        _, res = make_transfer(
            client=client, source=first_id, destination=second_id, amount=amount
        )
        assert res.status_code == 400
