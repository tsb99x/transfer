...
def error_response(status_code: int, error: str):
    return JSONResponse(
        ...
        content={"request_id": str(ctx_request_id.get()), "error": error},
    )
...
async def create_new_account(request: AccountBalance):
    ...
    await init_account(account_id=request.account_id, balance=request.balance)
...
async def get_account_balance(account_id: UUID):
    ...
    return AccountBalance(
        account_id=account_id, balance=metadata[account_id]["balance"]
    )
